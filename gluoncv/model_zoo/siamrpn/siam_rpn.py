...
class DepthwiseXCorr(HybridBlock):
    ...
    def hybrid_forward(self, F, kernel, search):
        ...
        search = search.reshape((1, -1, self.search_size[2], self.search_size[3]))
        kernel = kernel.reshape((-1, 1, self.kernel_size[2], self.kernel_size[3]))
        out = F.Convolution(data=search, weight=kernel, kernel=[self.kernel_size[2], self.kernel_size[3]],
                            no_bias=True, num_filter=batch*channel, num_group=batch*channel)
        out = out.reshape((batch, channel, self.out_size[2], self.out_size[3]))#[1, 256, 21, 21]
        out = self.head(out)
        return out
